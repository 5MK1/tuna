...
export default class CssColor {
    ...
    constructor(red: string, green: string, blue: string, alpha: string = '') {
    ...
    private static tryParseFromRgb(color: string): CssColor | undefined {
        ...
        const matches = /^rgb[a]?\((\d{1,3})\s*,\s*(\d{1,3})\s*,\s*(\d{1,3})\)$/i.exec(color);
        if (!matches) {
            return undefined;
        }

        const red = Number(matches[1]);
        if (red > 255) {
            return undefined;
        }

        const green = Number(matches[2]);
        if (green > 255) {
            return undefined;
        }

        const blue = Number(matches[3]);
        if (blue > 255) {
            return undefined;
        }

        return new CssColor(
            red === 0 ? '00' : red.toString(16),
            green === 0 ? '00' : green.toString(16),
            blue === 0 ? '00' : blue.toString(16)
        );
    }

    private static tryParseFromLiteral(color: string): CssColor | undefined {
        const lowerCaseColor = color.toLowerCase();
        const match = Object.entries(ColorNames)
            .filter(val => val[0] === lowerCaseColor)[0];
        if (!match) {
            return undefined;
        }
        return new CssColor(
            match[1].substring(1, 3),
            match[1].substring(3, 5),
            match[1].substring(5, 7)
        )
    }
}